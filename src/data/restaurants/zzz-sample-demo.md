---
name: サンプル軒（デモ用ダミー店舗）
country: デモ
region: 東京
area: 架空エリア・見本町
category: デモ用カテゴリ
description: 全項目の見本表示のために作成したダミーデータです。実在の店舗ではありません。
mapQuery: 東京駅
intro: >-
  これは詳細ページの全項目（基本情報・席数・招牌菜・写真・地図・リンク）がどのように表示されるかを確認するためのサンプルです。実際の店舗を掲載する際は、この構成に沿って各
  .md ファイルを編集してください。
tables: カウンター6席、テーブル4卓（各4名まで）、個室1室（最大8名）
signatureDishes:
  - デモ肉まん
  - サンプル炒飯
  - 見本デザート盛り合わせ
photos:
  - /images/restaurants/zzz-sample-demo/1.svg
  - /images/restaurants/zzz-sample-demo/2.svg
  - /images/restaurants/zzz-sample-demo/3.svg
hours: '11:00〜22:00（L.O. 21:30）'
closedDays: 毎週水曜日・年末年始
phone: 03-0000-0000（ダミー番号）
reservation: 予約推奨（電話・Web予約フォームどちらも可）
priceRange: 'ランチ ¥1,200〜／ディナー ¥3,000〜¥5,000'
dietary: ハラール・ベジタリアン対応可（要事前確認）
payment:
  - 現金
  - クレジットカード
  - PayPay等QR決済
website: 'https://example.com'
instagram: 'https://instagram.com/example'
x: 'https://x.com/example'
facebook: 'https://facebook.com/example'
tabelog: 'https://tabelog.com/example'
---
